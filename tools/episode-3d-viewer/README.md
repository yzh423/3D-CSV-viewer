...
4. 点击“重置视角”恢复自动相机。
5. 在“当前 Episode”中选择播放文件，使用播放/暂停、时间滑块、倍速和循环控制动画。
6. 红、绿、蓝三根短轴分别表示当前 TCP 工具坐标系的 X、Y、Z 方向；左右 TCP 可独立隐藏。
...
TCP 动画还需要对应侧的 `*_tcp_quat_w/x/y/z` 四元数字段。缺少四元数时仍可查看静态轨迹，但该侧不会生成工具坐标轴动画。
